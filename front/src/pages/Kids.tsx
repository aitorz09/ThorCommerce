...
export const Kids = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${url}/products/category/Kids`);
  ...
  const tagsArray = products.map((product:Product) => product.sizes.slice(0, 1));
    ...
    <>
      <Header/>
        ...
        <Link to={'/mujeres'}><img src={bannerWomen} alt="" /></Link>
          <h2 className='text-5xl my-10 text-center'>Niños</h2>
